...
@Component({
  selector: 'app-manage-nutrients',
  templateUrl: './manage-nutrients.component.html',
  styleUrls: ['./manage-nutrients.component.scss']
})
export class ManageNutrientsComponent implements OnInit,
  OnDestroy,
  ICreateAction,
  IEditAction,
  IDeleteAction,
  ...
  ngOnInit(): void {
    ...
    this.grouping = this.nutrientsService.grouping;
    this.paginator = this.nutrientsService.paginator;
    this.sorting = this.nutrientsService.sorting;
    const sb = this.nutrientsService.isLoading$.subscribe(res => this.isLoading = res);
  ...
  filter() {
    ...
    this.nutrientsService.patchState({ filter });
  }
  ...
  search(searchTerm: string) {
    this.nutrientsService.patchState({ searchTerm });
  }
  ...
  sort(column: string) {
    ...
    this.nutrientsService.patchState({ sorting });
  }

  // pagination
  paginate(paginator: PaginatorState) {
    this.nutrientsService.patchState({ paginator });
  }
  ...
  edit(id: number, referenceId?: number, languageId?: number) {
    const modalRef = this.modalService.open(EditNutrientsModalComponent, { size: 'xl' });
    if (referenceId) {
  ...
  delete(id: number) {
    ...
    modalRef.componentInstance.id = id;
    modalRef.componentInstance.service = this.nutrientsService;
    modalRef.componentInstance.fileName = 'Nutrients';
    modalRef.result.then(() => this.nutrientsService.fetch(), () => { });
  }
  ...
  updateStatusForSelected() {
    ...
    modalRef.componentInstance.ids = this.grouping.getSelectedRows();
    modalRef.componentInstance.tableName = 'Nutrients';
    modalRef.componentInstance.tableinfo = this.nutrientsService.items$;
    modalRef.componentInstance.serviceName = this.nutrientsService;
    modalRef.componentInstance.columnName = 'nutrientName';
    modalRef.result.then(() => this.nutrientsService.fetch(), () => { });
  }
  ...
  changeLanguage(e) {
    ...
    if (languageId == "null") {
      ...
      this.nutrientsService.fetchByLanguage(languageId);
    }
  }

  addNutrient(nutrient) {
    if (nutrient.referenceId) {
      this.edit(undefined, nutrient.referenceId, nutrient.languageId)
    }
    else {
      this.edit(undefined, nutrient.id, nutrient.languageId)
    }
  }
}

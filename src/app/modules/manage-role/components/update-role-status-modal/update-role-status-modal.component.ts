...
export class UpdateRoleStatusModalComponent implements OnInit {
  ...
  loadRoles() {
    const sb = this.rolesService.items$.pipe(
      first()
    ).subscribe((res: Roles[]) => {
      this.roles = res.filter(c => this.ids.indexOf(c.id) > -1);
    });
  ...
  updateRolesStatus() {
    this.isLoading = true;
    const sb = this.rolesService.updateStatusForItems(this.ids, this.status).pipe(
